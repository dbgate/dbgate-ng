...
module.exports = {
  ...
  async script({ packageName }) {
    ...
    if (packagedContent?.[packageName]) {
  ...
  async info({ packageName }) {
    // @ts-ignore
    const isPackaged = await fs.exists(
      path.join(packagedPluginsDir(), packageName)
    );

    try {
      const infoResp = await axios.default.get(
  ...
  async installed() {
    ...
    const files1 = packagedContent
      ? _.keys(packagedContent)
      : await fs.readdir(packagedPluginsDir());
      ...
      try {
        if (packagedContent?.[packageName]) {
          ...
          const manifest = await fs
            .readFile(
              path.join(
                isPackaged ? packagedPluginsDir() : pluginsdir(),
                packageName,
                "package.json"
              ),
              {
          ...
          if (
            !manifest.keywords.includes("dbgateplugin") &&
            !manifest.keywords.includes("dbgatebuiltin")
          ) {
            continue;
          }
          ...
          if (await fs.exists(readmeFile)) {
            manifest.readme = await fs.readFile(readmeFile, {
              encoding: "utf-8",
            });
          }
