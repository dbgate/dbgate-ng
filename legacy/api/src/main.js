...
function registerExpressStatic(app, publicDir) {
  app.get(
    [getExpressPath("/"), getExpressPath("/*.html")],
    async (req, res, next) => {
      try {
        const relPath =
          req.path === getExpressPath("/") ? "/index.html" : req.path;
...
function start() {
  ...
  if (platformInfo.isDocker) {
    ...
    app.get(getExpressPath("/"), (_req, res) => {
      res.send("DbGate API");
    });
  }
  ...
  app.get(getExpressPath("/stream"), async (req, res) => {
  ...
  app.get(getExpressPath("/health"), async (_req, res) => {
    res.setHeader("Content-Type", "application/json");
    const health = await getHealthStatus();
    res.end(JSON.stringify(health, null, 2));
  });

  app.get(getExpressPath("/__health"), async (_req, res) => {
    res.setHeader("Content-Type", "application/json");
  ...
  if (platformInfo.isDocker) {
    ...
    logger.info(
      `DBGM-00028 DbGate API listening on port ${port} (docker build)`
    );
    server.listen(port);
  } else if (platformInfo.isAwsUbuntuLayout) {
    const port = process.env.PORT || 3000;
    logger.info(
      `DBGM-00029 DbGate API listening on port ${port} (AWS AMI build)`
    );
    server.listen(port);
  } else if (platformInfo.isAzureUbuntuLayout) {
    const port = process.env.PORT || 3000;
    logger.info(
      `DBGM-00030 DbGate API listening on port ${port} (Azure VM build)`
    );
    server.listen(port);
  } else if (platformInfo.isNpmDist) {
    getPort({
    ...
    const port = process.env.PORT || 3000;
    logger.info(
      `DBGM-00032 DbGate API & web listening on port ${port} (dev web build)`
    );
    server.listen(port);
  } else {
    const port = process.env.PORT || 3000;
    logger.info(
      `DBGM-00033 DbGate API listening on port ${port} (dev API build)`
    );
    server.listen(port);
  }
