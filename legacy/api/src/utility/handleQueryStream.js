...
class QueryStreamTableWriter {
  ...
  initializeFromQuery(
    ...
    if (isProApp() && (chartDefinition || autoDetectCharts)) {
      this.chartProcessor = chartDefinition
        ? new ChartProcessor([chartDefinition])
        : new ChartProcessor();
    }
    process.send({
      msgtype: "recordset",
  ...
  row(row) {
    ...
    this.currentStream.write(
      `${JSON.stringify(row, serializeJsTypesReplacer)}\n`
    );
    try {
  ...
  row(row) {
    ...
    if (this.limitRows && this.rowCounter >= this.limitRows) {
      process.send({
        msgtype: "info",
        info: {
          message: `Rows limit overflow, loaded ${this.rowCounter} rows, canceling query`,
          severity: "error",
        },
        sesid: this.sesid,
      });
