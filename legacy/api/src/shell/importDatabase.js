...
class ImportStream extends stream.Transform {
  ...
  async _transform(chunk, _encoding, cb) {
    try {
      if (chunk.specialMarker === "copy_stdin_start") {
        this.writeQueryStream = await this.driver.writeQueryFromStream(
          this.dbhan,
          chunk.text
        );
        ...
        await this.driver.script(this.dbhan, chunk.text, {
          queryOptions: { importSqlDump: true },
        });
      }
    } catch (err) {
