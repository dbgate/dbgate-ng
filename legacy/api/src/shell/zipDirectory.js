...
function zipDirectory(inputDirectory, outputFile) {
  if (outputFile.startsWith("archive:")) {
    outputFile = path.join(
      archivedir(),
      outputFile.substring("archive:".length)
    );
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    output.on("close", () => {
      logger.info(
        `DBGM-00072 ZIP file created (${archive.pointer()} total bytes)`
      );
      resolve();
    });
    ...
    archive.on("warning", (err) => {
      logger.warn(
        ...
        `DBGM-00073 Warning while creating ZIP: ${err.message}`
      );
    });
    ...
    archive.on("error", (err) => {
      logger.error(
        extractErrorLogData(err),
        `DBGM-00074 Error while creating ZIP: ${err.message}`
      );
      reject(err);
    });
