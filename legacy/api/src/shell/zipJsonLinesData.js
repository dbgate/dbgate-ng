...
function zipDirectory(jsonDb, outputFile) {
  ...
  return new Promise((resolve, reject) => {
    ...
    output.on("close", () => {
      logger.info(
        `DBGM-00075 ZIP file created (${archive.pointer()} total bytes)`
      );
      resolve();
    });
    ...
    archive.on("warning", (err) => {
      logger.warn(
        ...
        `DBGM-00076 Warning while creating ZIP: ${err.message}`
      );
    });
    ...
    archive.on("error", (err) => {
      logger.error(
        extractErrorLogData(err),
        `DBGM-00077 Error while creating ZIP: ${err.message}`
      );
      reject(err);
    });
