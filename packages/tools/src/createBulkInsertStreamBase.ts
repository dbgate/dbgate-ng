import type { EngineDriver, WriteTableOptions } from "dbgate-types";
...
export function createBulkInsertStreamBase(
  driver: EngineDriver,
  stream,
  dbhan,
  name,
  ...
  writable.checkStructure = async () => {
    try {
      let structure =
        options.targetTableStructure ??
        (await driver.analyseSingleTable(dbhan, name));
      if (structure) {
      ...
      if (options.createIfNotExists && (!structure || options.dropIfExists)) {
        ...
        const createdTableInfo = driver.adaptTableInfo(
          prepareTableForImport({ ...writable.structure, ...name })
        );
        dmp.createTable(createdTableInfo);
        logger.info(
          { sql: dmp.s },
          `DBGM-00124 Creating table ${fullNameQuoted}`
        );
      ...
      if (!writable.structure) {
        throw new Error(
          `DBGM-00125 Error importing table - ${fullNameQuoted} not found`
        );
      }
      if (options.truncate) {
      ...
      writable.columnDataTypes = _fromPairs(
        writable.columnNames.map((colName) => [
          colName,
          writable.structure.columns.find((x) => x.columnName === colName)
            ?.dataType,
        ])
      );
    } catch (err) {
      logger.error(
        ...
        "DBGM-00184 Error during preparing bulk insert table, stopped"
      );
  ...
  writable.send = async () => {
    ...
    try {
      ...
      logger.error(
        extractErrorLogData(err),
        "DBGM-00185 Error during base bulk insert, insert stopped"
      );
      writable.destroy(err);
    }
  };
