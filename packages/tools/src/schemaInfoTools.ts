import type { SchemaInfo, SqlDialect } from "dbgate-types";

export function findDefaultSchema(
  schemaList: SchemaInfo[],
  dialect: SqlDialect,
  schemaInStorage: string = null
) {
  if (!schemaList) {
    return null;
  }
  ...
  if (
    schemaInStorage &&
    schemaList.find((x) => x.schemaName === schemaInStorage)
  ) {
  ...
  if (
    dialect?.defaultSchemaName &&
    schemaList.find((x) => x.schemaName === dialect.defaultSchemaName)
  ) {
