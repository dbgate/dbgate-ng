...
export class SqlDumper implements AlterProcessor {
  ...
  static convertKeywordCase(keyword: any): string {
    if (SqlDumper.keywordsCase === "lowerCase")
  ...
  putByteArrayValue(_value) {
    this.put("^null");
  }
  putValue(value, _dataType = null) {
    ...
    else if (value?.type === "Buffer" && _isArray(value?.data))
      this.putByteArrayValue(value?.data);
    else if (value?.$bigint) this.putRaw(value?.$bigint);
    else if (_isPlainObject(value) || _isArray(value))
  ...
  specialColumnOptions(_column) {}

  selectScopeIdentity(_table: TableInfo) {}
    ...
    if (this.dialect?.defaultValueBeforeNullability) {
      ...
      if (includeNullable && !this.dialect?.specificNullabilityImplementation) {
        this.put(
          column.notNull
            ? "^not ^null"
            : this.dialect.implicitNullDeclaration
              ? ""
              : "^null"
        );
      }
  ...
  columnDefault(column: ColumnInfo) {
    if (
      ...
      this.dialect?.namedDefaultConstraint
    ) {
      this.put(
  ...
  tableOptions(table: TableInfo) {
    const options =
      this.driver?.dialect?.getTableFormOptions?.("sqlCreateTable") || [];
  ...
  createTablePrimaryKeyCore(table: TableInfo) {
    if (table.primaryKey) {
      ...
      if (
        ...
        !this.dialect.anonymousPrimaryKey
      ) {
  ...
  transform(_type: TransformType, dumpExpr) {
    dumpExpr();
  }

  allowIdentityInsert(_table: NamedObjectInfo, _allow: boolean) {}
  enableConstraints(_table: NamedObjectInfo, _enabled: boolean) {}
  enableAllForeignKeys(_enabled: boolean) {}
  ...
  changeViewSchema(_obj: ViewInfo, _newSchema: string) {}
  renameView(_obj: ViewInfo, _newSchema: string) {}
  ...
  changeMatviewSchema(_obj: ViewInfo, _newSchema: string) {}
  renameMatview(_obj: ViewInfo, _newSchema: string) {}
  ...
  alterProcedure(obj: ProcedureInfo) {
    this.putRaw(
      obj.createSql.replace(/create\s+procedure/i, "ALTER PROCEDURE")
    );
    this.endCommand();
  }
  changeProcedureSchema(_obj: ProcedureInfo, _newSchema: string) {}
  renameProcedure(_obj: ProcedureInfo, _newSchema: string) {}
  ...
  changeFunctionSchema(_obj: FunctionInfo, _newSchema: string) {}
  renameFunction(_obj: FunctionInfo, _newSchema: string) {}
  ...
  changeTriggerSchema(_obj: TriggerInfo, _newSchema: string) {}
  renameTrigger(_obj: TriggerInfo, _newSchema: string) {}
  ...
  dropConstraintCore(cnt: ConstraintInfo) {
    this.putCmd(
      "^alter ^table %f ^drop ^constraint %i",
      cnt,
      cnt.constraintName
    );
  }
  ...
  createIndex(ix: IndexInfo) {
    ...
    this.putCollection(",&n", ix.columns, (col) => {
      this.put(
        "%i %k",
        col.columnName,
        col.isDescending === true ? "DESC" : "ASC"
      );
    });
    this.put("&<&n)");
  ...
  renameColumn(_column: ColumnInfo, _newName: string) {}
  ...
  changeTableSchema(_obj: TableInfo, _schema: string) {}

  renameTable(_obj: TableInfo, _newname: string) {}

  renameSqlObject(_obj: SqlObjectInfo, _newname: string) {}
  ...
  selectTableIntoNewTable(
    sourceName: NamedObjectInfo,
  ...
  recreateTable(oldTable: TableInfo, newTable: TableInfo) {
    if (
      !oldTable.pairingId ||
      !newTable.pairingId ||
      oldTable.pairingId !== newTable.pairingId
    ) {
      throw new Error(
        "Recreate is not possible: oldTable.paringId != newTable.paringId"
      );
    }
  ...
  dropSqlObject(obj: SqlObjectInfo) {
    this.putCmd(
      "^drop %s %f",
      this.getSqlObjectSqlName(obj.objectTypeField),
      obj
    );
  }
  ...
  setTableOption(table: TableInfo, optionName: string, optionValue: string) {
    ...
    this.setTableOptionCore(
      table,
      optionName,
      optionValue,
      option.sqlFormatString
    );
  ...
  fillNewNotNullDefaults(col: ColumnInfo) {
    if (col.notNull && col.defaultValue != null) {
      this.putCmd(
        "^update %f ^set %i = %s ^where %i ^is ^null",
        col,
        col.columnName,
        col.defaultValue,
        col.columnName
    ...
    for (const row of newRows) {
      ...
      if (old) {
        ...
        if (updated.length > 0) {
          ...
          this.putCollection(", ", updated, (col) =>
            this.put("%i=%v", col, row[col])
          );
          this.put(" ^where ");
          this.putCollection(" ^and ", key, (col) =>
            this.put("%i=%v", col, row[col])
          );
        }
      } else {
