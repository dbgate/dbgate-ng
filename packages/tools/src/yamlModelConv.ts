...
function columnInfoToYaml(
  ...
  const fk = table.foreignKeys?.find(
    (fk) =>
      fk.columns.length === 1 && fk.columns[0].columnName === column.columnName
  );
  if (
    fk &&
...
function columnInfoFromYaml(
  column: ColumnInfoYaml,
  table: TableInfoYaml
): ColumnInfo {
  const res: ColumnInfo = {
    ...
    dataType: column.length
      ? `${column.type}(${column.length < 0 ? "max" : column.length})`
      : column.type,
    autoIncrement: column.autoIncrement,
    notNull: column.notNull || table.primaryKey?.includes(column.name),
    defaultValue: column.default,
    defaultConstraint:
      column.default != null ? `DF_${table.name}_${column.name}` : undefined,
  };
  return res;
}
    ...
    res.indexes = tableCopy.indexes.map((index) => {
      const idx: IndexInfoYaml = {
        ...
        columns: index.columns
          .filter((x) => !x.isIncludedColumn)
          .map((x) => x.columnName),
      };
      if (index.columns.some((x) => x.isIncludedColumn)) {
        idx.included = index.columns
          .filter((x) => x.isIncludedColumn)
          .map((x) => x.columnName);
      }
      return idx;
...
export function tableInfoFromYaml(
  table: TableInfoYaml,
  allTables: TableInfoYaml[]
): TableInfo {
  const res: TableInfo = {
    ...
    foreignKeys: _compact(
      table.columns
        ...
        .map((col) => convertForeignKeyFromYaml(col, table, allTables))
    ),
...
export function databaseInfoFromYamlModel(
  filesOrDbInfo: DatabaseModelFile[] | DatabaseInfo
): DatabaseInfo {
  ...
  model.tables = tablesYaml.map((table) =>
    tableInfoFromYaml(table, tablesYaml)
  );
