...
const FontIcon: Component<FontIconProps> = (props) => {
  ...
  const [local, others] = splitProps(mergedProps, ['icon', 'title', 'padLeft', 'padRight', 'style', 'onClick']);
  ...
  const getClassName = () => {
    const baseClass = iconNames[local.icon] || local.icon;
    const classes = [baseClass];
    if (local.padLeft) classes.push(styles['pad-left']);
    if (local.padRight) classes.push(styles['pad-right']);
    return classes.join(' ');
  };
